...
beforeEach(async () => {
    ...
    await instancia.setService("Servicio 0001", 0,"https://localhost/servicio001", 3, 3)
    await instancia.setService("Servicio 0002", 0,"https://localhost/servicio002", 3, 3)
    await instancia.setService("Servicio 0003", 0,"https://localhost/servicio003", 3, 3)
...
contract('MissionsSoS', accounts => {
    ...
    it('Crear constituyentes con los datos adecuados en las posiciones adecuadas', async() =>{       
        assert.equal((await instancia.getConstituent(3))[0], 3, "El código del tercer constituyente no coincide")
    });
    ...
    it('Crear parámetros en los constituyentes e ir actualizando el total de los parámetros por constituyente', async() =>{       
        ...
        await instancia.setConstituentParameter(3, "clave 3.1", "valor 3.1", "descripción 3.1" )
        await instancia.setConstituentParameter(3, "clave 3.2", "valor 3.2", "descripción 3.2" )
        await instancia.setConstituentParameter(3, "clave 3.3", "valor 3.3", "descripción 3.3" )  
        assert.equal(await instancia.totalConstituentParameter(1), 1, "Total de parámetros del constituyente no coinciden")
        assert.equal(await instancia.totalConstituentParameter(2), 2, "Total de parámetros del constituyente no coinciden")
    ...
    it('Crear parámetros en los constituyentes con los datos adecuados en las posiciones adecuadas', async() =>{       
        ...
        assert.equal((await instancia.getConstituentParameter(1,0))[0], "clave 1.1", "La clave del primer constituyente no coincide")
        assert.equal((await instancia.getConstituentParameter(2,1))[1], "valor 2.2", "El valor del segundo constituyente no coincide")
